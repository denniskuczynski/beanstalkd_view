...
function peek(queue) {
  ...
  var jqxhr = $.getJSON("/beanstalkd/peek/"+tube+"/"+queue, function(data) {
    if (data.hasOwnProperty("error")) {
      alert(data["error"]);
    } else {      
      $("#job_info_popup_body").html(create_job_info_table(data));
      $("#job_info_popup_footer").html(create_job_info_buttons(tube, data));
...
function create_job_info_buttons(tube, data) {
  var job_id = data["id"];
  var priority = data["pri"];
  var job_info_buttons = "";
  job_info_buttons += "<a href=\"/beanstalkd/delete/"+tube+"/"+job_id+"\" class=\"btn btn-primary\">Delete Job</a>";
...
function create_job_info_table(data) {
  ...
  job_info_table += create_job_info_row("id", data["id"], "The job id");      
  job_info_table += create_job_info_row("tube", data["tube"], "The name of the tube that contains this job");      
  job_info_table += create_job_info_row("state", data["state"], "is \"ready\" or \"delayed\" or \"reserved\" or \"buried\"");      
  ...
  job_info_table += create_job_info_row("ttr", data["ttr"], "");      
...
function create_job_info_row(key, value, title) {
  var job_info_row = "<tr>";
  job_info_row += "<td rel=\"tooltip\" title=\""+title+"\">"+key+"</td>";
